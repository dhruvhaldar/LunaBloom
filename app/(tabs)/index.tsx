...
export default function HomeScreen() {
  ...
  const logPeriod = useCallback(async () => {
    ...
    // Security: Sanitize and validate inputs before processing
    const sanitizedNotes = sanitizeInput(notes);

    if (containsSuspiciousPatterns(sanitizedNotes)) {
        Alert.alert(
            "⚠️ Invalid Input",
            "Your notes contain characters or patterns that are not allowed for security reasons. Please remove any special characters or scripts.",
            [{ text: "OK" }]
        );
        return;
    }

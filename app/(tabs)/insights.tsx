...
export default function InsightsScreen() {
  ...
  // Optimization: Memoize label component to prevent re-creation on every render
  // Inline components in Victory cause expensive re-calculations
  const labelStyle = useMemo(() => ({ fontSize: 13, fill: barColor }), [barColor]);
  const chartStyle = useMemo(() => ({ data: { fill: barColor }, labels: { fill: barColor } }), [barColor]);

  // Stable label text function
  const getLabelText = useCallback(({ datum }: any) => `${datum.dateRange.split(' - ')[0]} (${datum.y} days)`, []);
      ...
      <ThemedView style={styles.container}>
        ...
        <ThemedView style={styles.sectionContainer}>
          ...
          {cycleData.length > 0 ? (  
            ...
            labels={({ datum }) => `${datum.y} days`} // Still needed for some Victory internals, but labelComponent takes precedence
